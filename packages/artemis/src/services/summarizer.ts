/** biome-ignore-all lint/style/noNonNullAssertion: Acceptable here */
...
import { MemberCache } from '../core/member-cache.ts';
import { Messages } from '../core/messages.ts';

// biome-ignore lint/complexity/noBannedTypes: Acceptable here
...
export class Summarizer extends Effect.Service<Summarizer>()('app/Summarizer', {
	scoped: Effect.gen(function* () {
		...
		const summarizeThread = Effect.fn('Summarizer.summarizeThread')(function* (
			...
			const threadMessages = yield* Stream.runCollect(messages.cleanForChannel(channel)).pipe(
				Effect.map(Chunk.reverse)
			);
			return yield* summarize(
				parentChannel as Discord.GuildChannelResponse,
				channel,
				threadMessages,
				small
			);
		...
		const summarizeWithMessages = (
			...
			pipe(
				...
				Effect.flatMap((parentChannel) =>
					summarize(parentChannel as Discord.GuildChannelResponse, channel, messages, small)
				),
			...
			Effect.forEach(
				...
				(message, index) => {
					const reply = pipe(
						...
						Option.map((index) => [Chunk.unsafeGet(messages, index), index + 1] as const)
					);
...
Thread started at: ${new Date(thread.thread_metadata!.create_timestamp!).toUTCString()}
...
${messageContent.join('\n\n')}`
		...
		const summarizeMessage = Effect.fn('Summarizer.summarizeMessage')(function* (
			thread: Discord.ThreadResponse,
			index: number,
...
${message.content}${imagesContent}`;
	...
	dependencies: [ChannelsCache.Default, MemberCache.Default, Messages.Default, DiscordGatewayLayer],
}) {}
